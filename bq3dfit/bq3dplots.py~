...
# ------------
def get_axFig(fig=None, axFig=None, visible=False):

    if fig is None:
        fig = plt.gcf()

    if axFig is None:
        axFig = fig.add_subplot(111, zorder=-99)

    # Properties
    axFig = invisible_axis(axFig, visible)
    return axFig

def invisible_axis(axis, visible=False):
...
def plot_single_fit(filename_q3di, col, row, wvel_line=None, rest=True,
                    ...
                    show=True, 
                    plot_data_opts={}, plot_model_opts={},
                    plot_comp_opts={}, plot_guess_opts={}, plot_cont_opts={},
                    filename_npz=None, cmap_comp=None,
                    **args):
    '''
    Input
    -----
    ...
    #plot_data_opts1 = dict(color='C0', alpha=.7, lw=1.5)
    #plot_model_opts1 = dict(color='C1')#color='C0', alpha=.7, lw=1.5)
    #plot_comp_opts1 = dict(color='k', ls="-", lw=.7, alpha=.7)
    #plot_guess_opts1 = dict(color='C2')#, alpha=.7, lw=1.5)
    #plot_cont_opts1 = dict(color='yellow')

    #cmap_comp = tol_colors.tol_cmap('rainbow_PuBr')

    plot_data_opts1 = dict(color='k', alpha=1, lw=1.5)
    plot_model_opts1 = dict(color='C0')
    plot_guess_opts1 = dict(color='C1')
    plot_cont_opts1 = dict(color='gray')

    plot_data_opts1.update(args)
    plot_data_opts1.update(plot_data_opts)
    plot_model_opts1.update(plot_model_opts)
    plot_guess_opts1.update(plot_guess_opts)
    plot_cont_opts1.update(plot_cont_opts)
    ...
    if plot_model:
        ...
        n_gauss = q3do.maxncomp

        if filename_npz is None:
            ...
            lines = q3do.linelabel
            p = []

            for line in lines:
                for comp in range(n_gauss):

                    fluxpk = linedat['emlflx'].item()[f'fc{comp+1}pk'][line][col-1, row-1]
                    wav = linedat['emlwav'].item()[f'c{comp+1}'][line][col-1, row-1]
                    sig = linedat['emlsig'].item()[f'c{comp+1}'][line][col-1, row-1]
                    #print(f'{line} {comp} {fluxpk} {wav} {sig}')
                    p += [fluxpk, wav, sig]
            p = np.array(p)
            ...
            mask = np.isnan(pp[:,0])
            pp[mask] = [0, q3do.wave[0], 0]

        ncomp_tot = len(specres)
        nlines = len(q3do.linelabel)
        if (ncomp_tot / nlines) != float(n_gauss):
            ncomp_tot = nlines * n_gauss
            specres = [specres[1]] * ncomp_tot
            #assert (ncomp_tot / nlines) == float(n_gauss)
        ...
        if cmap_comp is None:
            cmap_comp = plt.cm.plasma_r
        colors_comp = list(cmap_comp(np.linspace(0, 1, n_gauss)))
        plot_comp_opts1 = dict(color='k', ls="-",  lw=1.5, alpha=1)
        if 1:
            plot_comp_opts1['color'] = colors_comp

        plot_comp_opts1.update(plot_comp_opts)


        if not bq3dutils.is_iterable(plot_comp_opts1['color']):
            colors_comp = ncomp_tot * [plot_comp_opts1['color']]
            plot_comp_opts1['color'] = colors_comp
        else:
            assert n_gauss <= len(plot_comp_opts1['color']), \
                    f"{n_gauss}, {len(plot_comp_opts1['color'])}"

            colors_comp = nlines * list(plot_comp_opts1['color'][:n_gauss])
            plot_comp_opts1['color'] = colors_comp
            #assert ncomp <= len(plot_comp_opts1['color']), \
            #        f"{ncomp}, {len(plot_comp_opts1['color'])}"
    ...
    # data
    ax1.__getattribute__(plottype_data)(spectral, spec, label="data", 
                                        **plot_data_opts1)
    ...
    if plot_model:
        ...
        ax1.plot(spectral, model, label="model", **plot_model_opts1)

        # individual components
        colors_comp = plot_comp_opts1.pop('color')
        labels = [f'c{comp}' for comp in np.arange(1, n_gauss+1)]
        labels += [''] * (ncomp_tot - n_gauss)
        for comp, (imodel, imodel0) in enumerate(zip(model_comps, model_comps0)):
            ax1.plot(spectral, cont_fit + imodel, color=colors_comp[comp], 
                     label=labels[comp],
                     **plot_comp_opts1)
    ...
    if plot_cont:
        ax1.plot(spectral, cont_fit, label="continuum", **plot_cont_opts1)
    ...
    if plot_error:
        ...
        if filename_noise is None:
            filename_noise = q3di.filename_noise
        ...
        if 1:
            with fits.open(filename_noise) as hdu_noise:
                opts_cont_line = dict(color='blue', alpha=.7, lw=.5, ls=':',
                                        label="3*stddev in cont. region")
                if len(hdu_noise) == 1:
                    noise2D = hdu_noise[0].data

                    cont_std = noise2D[col-1, row-1] / q3do.fluxnorm
                    ...
                    ax1.axhline(cont_std*3, **opts_cont_line)

                else:
                    noise_spec = hdu_noise['polyfit'].data / q3do.fluxnorm

                    noise_wl = bq3dutils.get_spectral(hdu_noise['polyfit'].header).to(q3do.waveunit).value
                    noise_wl = (noise_wl / (1 + redshift)) 

                    noise_2D = bq3dutils.read_quantity(hdu_noise, 'polyfit2D').value

                    noise_2D_xy = noise_2D.T[col-1, row-1]
                    noise_spec = noise_spec * noise_2D_xy

                    if wvel_line is not None:
                        noise_wl = (noise_wl / rest_line - 1) * c_kms
                    idx_match = (noise_wl >= spectral[0]) & (noise_wl <= spectral[-1])

                    ax1.plot(noise_wl[idx_match], noise_spec[idx_match]*3,
                                      **opts_cont_line)

                    #cont_std = np.nanmean(noise_spec*3)
        ...
        # == residuals == 
        plot_residual_opts1 = {}
        plot_residual_opts1.update(plot_data_opts1)
        plot_residual_opts1['alpha'] = .3 
        ax2.fill_between(spectral, -spec_err, spec_err, **plot_residual_opts1)
    ...
    if filename_npz is None:
        ...
        pp2 = pp.reshape((nlines, n_gauss, 3))
        pp2[:,:,2]

        Amp_fit = pandas.DataFrame(pp2[:,:,0].T, columns=lines)
        wobs_fit = pandas.DataFrame(pp2[:,:,1].T, columns=lines)
        sig_wave_fit = pandas.DataFrame(pp2[:,:,2].T, columns=lines)

        sig_fit = sig_wave_fit * c_kms.value / wobs_fit
        vel_fit = (wobs_fit/(1+redshift)/linelist['lines']-1) * c_kms
        
        vel_fit[sig_fit == 0] = np.nan
        print(np.isnan(sig_fit))
    ...
    with pandas.option_context('display.float_format', '{:.5g}'.format):
        print(pars)
...
def __call_plot_interactive(q3di=None, opts_load_maps={}, q3di_continuum=None,
...
def call_plot_interactive(q3di=None, opts_load_maps={}, q3di_continuum=None,
    ...
    # Load linemaps
    linemaps = bq3dutils.load_linemaps_all(q3di, **opts_load_maps)

    # Add general data
    if 1:
        key_list_general = ['redchisq', 'maxncomp', 'ndata', 'aic0', 'bic0']
        line_ref = linenames[0]
        #linemaps_ref = bq3dutils.load_linemaps(q3di, line_ref, **opts_load_maps)
        for key in key_list_general:

            img_dict[f'{key}'] = getattr(linemaps[line_ref], key)
