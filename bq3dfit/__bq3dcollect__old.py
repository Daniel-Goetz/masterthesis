...
def q3dcollect(rank, size, q3di, cols=None, rows=None, quiet=True, compsortpar='sigma',
    ...
    if 1:
        ...
        if 1:
            ...
            colarri = colarr[start:stop]
            rowarri = rowarr[start:stop]
    ...
    for ispax in range(0, nspaxi):
        ...
        if not quiet:
            print(f'core {rank} | Column {i+1} of {cube.ncols}')
...
#                                = lineweqs['comp'][line].data[sindex]
    ...
    if q3di.dolinefit:
        outfile = '{0.outdir}{0.label}'.format(q3di) + '.line.' + f'core{rank}'
        np.savez(outfile,
                 ...
                 #ndata=ndata, dof=dof, redchisq=redchisq
                 )
        print('q3dcollect: Saving emission-line fit results into '+outfile)
    if q3di.docontfit:
        outfile = '{0.outdir}{0.label}'.format(q3di)+'.cont.npz'
        np.save(outfile, contcube)
        print('q3dcollect: Saving continuum fit results into '+outfile)

def q3dcollecst(q3di, ncores):

    files = ['{0.outdir}{0.label}'.format(q3di)+'.line.' + f'core{rank}.npz' for i in range(ncores)]
    ...
    data0 = np.load(files[0], allow_pickle=True)    
    keys = data0.files
...
if __name__ == "__main__":
    ...
    q3dcollect(rank, size, q3difile, cols, rows)
