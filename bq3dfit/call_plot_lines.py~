...
def plot_maps_line(filename_q3di, cube=None, filename_npz=None, output_dir=None,
    ...
    if opts_load_maps['filename_npz'] is None:
        ...
        # Continuum - Contour
        title = 'Optical cont. /\n radio cont. (15 GHz)'
        plots.plot_quantity_map(img_cont, axis=ax[-2,0],  doLog=True, 
                                    X=dra,Y=ddec,
                                        vmin=400, vmax=None, vpercent=[1,100],
                                        do_contour=True,
                                        do_image=True,
                                        contour_opts=dict(nlevels=10, colors='white'),
                                        do_colorbar=True,
    ...
    fig.savefig(f'{output_dir}/maps_{label}.pdf', bbox_inches='tight')
